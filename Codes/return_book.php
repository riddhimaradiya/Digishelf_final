...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $mem_rt_date = isset($_POST['mem_rt_date']) ? $_POST['mem_rt_date'] : '';

    if ($transaction_id && $mem_rt_date) {
        $message = $returnBook->returnBook($transaction_id, $mem_rt_date);
        // Refresh the books list after return
        $books = $returnBook->getBooksToReturn();
    } else {
        $message = "Please select a book and enter a return date.";
    }
}
?>
...
<body style="background: linear-gradient(120deg, #f8f9fa 60%, #e9ecef 100%); font-family: 'Segoe UI', 'Roboto', Arial, sans-serif;">
    <div class="container py-5">
        <div class="row justify-content-center">
            <div class="col-lg-7">
                <div class="card shadow-lg border-0 rounded-lg">
                    <div class="card-body">
                        <h2 class="text-center page-title mb-4">Return Book</h2>
                        ...
                        <form method="POST" class="mt-3">
                            <div class="mb-3">
                                ...
                                <select class="form-select w-100" id="transaction_id" name="transaction_id" required style="min-width:200px;">
                                    <option value="" disabled selected>Select a book</option>
                                    <?php foreach ($books as $book) { ?>
                                    <option value="<?= $book['br_id'] ?>">
                                        <?= htmlspecialchars($book['book_title']) ?> (Issued: <?= date('d M Y', strtotime($book['issue_date'])) ?>)
                                    </option>
                                    <?php } ?>
                                </select>
                            </div>
